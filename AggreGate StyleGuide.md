...
# Именование
## Используйте camelCase стиль для именования сущностей
...
Все составные слова пишутся слитно, без пробелов, каждое слово, кроме первого, начинается с заглавной буквы.
...
Имя должно быть понятным и отображать суть сущность (устройство, модель, переменная и т.д.). Правильное значительно облегчит работу вам, и тем кто придёт после вас.
...
Любая функция или набор правил всегда выполняют определенное действие. Именно поэтому, их имя должно отвечать на вопрос “Что делает эта функция/набор правил?”.
...
По комментариям в наборах правил можно быстро понять назначения отдельного шага, и общую структуру сценария.
...
## Не оставляйте закомментированные фрагменты кода в выражении, и лишние сущности в дереве и моделях
Иногда для отладки выражения приходится заменять переменные окружения на прямые ссылки. Так как процесс отладки может занимать несколько итераций в коде выражения часто присутствуют сразу два варианта исполнения, один из которых закомментирован. Подобные куски кода надо обязательно убирать по окончанию отладки. Это потенциальная уязвимость приложения, особенно, если в них содержатся важные данные, например имена пользователей, пароли, IP адреса.
Также нельзя оставлять контексты, переменные, функции и другие сущности, которые требовались на этапе разработке, но не применяются при эксплуатации приложения. Наличие таких сущностей значительно усложняет сопровождение приложения.

# Разработка
## Создавайте справочники вместо прямых ссылок и константы
...
В языке выражений для большинства функций надо указывать параметры. В описании каждой функции указаны ее параметры, записанные в одну строку через запятую. Если параметров меньше трех и они достаточно короткие, то допустимо в коде выражения писать их в одну строку. Однако, в большинстве случаев лучше указать параметры в столбик. Это значительно облегчает работу с функцией и делает ее более читаемой. 
Разделители строки (знаки “,” или “+”) необходимо ставить в начале новой строки. Это поможет понять где начинается новый параметр и облегчит работу с кодом.
...
**Замечание**
Иногда вместо использование языка выражений лучше написать скрипт на Java. Например, для организации цикла или обработки большого массива данных. При этом желательно, чтобы все необходимые для работы скрипта данные передавались в качестве входных параметров, а результаты работы возвращались в качестве выходных параметров. Лучше не применять Java для модификации контекстов, или выполнения других низкоуровневых операций.
...
## Функция sleep
Как правило использование функции sleep является плохой практикой. Вместо этого старайтесь реагировать на события, в крайнем случае можно сделать цикл, применив функцию aggregate и catch, установив минимальный таймаут.
...
## Константы в строке
Если при работе выражения необходимо использовать константы (ip, имя контекста, переменная, множитель и т.д.), лучше не помещать их внуть выражения, а сохранить в переменную и использовать ссылку.

## Не писать документацию
Отсутствие документации - очень плохая практика! Она всё равно понадобится, и проще написать её сразу.
