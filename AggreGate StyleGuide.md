...
``` java
aggregate(
	table("<M=100>")  //Создание пустой таблицы с 100 записей
	,"catch({.:time}, sleep(10))" //Если при попытке получить значение переменной time будет ошибка, выполнится функция sleep(10). После 100 неудачных попыток вернётся null.
	,0
)
```
